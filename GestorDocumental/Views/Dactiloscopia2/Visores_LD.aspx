...
<head runat="server" title="Visores">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            setTimeout("jsRemoveWindowLoadVLD()", 3000);
        });

        function jsRemoveWindowLoadVLD() {
            // eliminamos el div que bloquea pantalla
            $("#WindowLoad").remove();

        }

        function jsShowWindowLoadVLD(mensaje) {
            //eliminamos si existe un div ya bloqueando
            jsRemoveWindowLoadVLD();
        ...
        var neg;
        neg = '../../../Content/ArchivosCliente/<%=Session["negVisorLD"]%>/<%=Session["negVisorLD"]%>.pdf';

        // creacion de los cuatro visores de pdf que se usaran en el modulo

        function inicioPDF1() {
            if ('<%=Session["Pagina20"]%>' == '') {
                neg = '../../../Content/ArchivosCliente/<%=ViewData["negVisorLD"]%>/<%=ViewData["negVisorLD"]%>.pdf';
            } else {
                neg = '../../../Content/ArchivosCliente/<%=ViewData["negVisorLD"]%>/Pagina-<%=Session["Pagina20"]%>.pdf';
            }
        ...
        function inicioPDF2() {
            ...
            if ('<%=Session["Pagina280"]%>' == '') {
                neg = '../../../Content/ArchivosCliente/<%=ViewData["negVisorLD"]%>/<%=ViewData["negVisorLD"]%>.pdf';
            } else {
                neg = '../../../Content/ArchivosCliente/<%=ViewData["negVisorLD"]%>/Pagina-<%=Session["Pagina280"]%>.pdf';
            }
        ...
        function inicioPDF3() {
            ...
            neg = '../../../Content/ArchivosCliente/<%=ViewData["negHoja_3"]%>/page-8.pdf';
            ...
            /*si el producto es LD pone posiciones del pagare LD*/
            document.getElementById('Iframep3').contentWindow.cordenadasPDFValDacti(pag1, 540, 600, 2.2);
        ...
        function RutaNegocio() {
            ...
            if (contador == 1) {
                if ('<%=Session["Pagina20"]%>' != '0') {
                    contador++;
                    return '../../../Content/ArchivosCliente/<%=ViewData["negVisorLD"]%>/Pagina-<%=Session["Pagina20"]%>.pdf';
                }
                else {
                    contador++;
                    Alternar(NoCedula);
                    return '../../../Content/ArchivosCliente/<%=ViewData["negVisorLD"]%>/<%=ViewData["negVisorLD"]%>.pdf';
                }
            } else if (contador == 2 || contador == 3) {
                if ('<%=Session["Pagina280"]%>' != '0') {
                    contador++;
                    return '../../../Content/ArchivosCliente/<%=ViewData["negVisorLD"]%>/Pagina-<%=Session["Pagina280"]%>.pdf';
                }
                else {
                    contador++;
                    Alternar(NoPagare1);

...
//var DEFAULT_URL = 'C:/Users/Administrator/Documents/Visual Studio 2012/Projects/BcoPopularConsumo/GestorDocumental/Content/ArchivosCliente/144326/144326.pdf';
var DEFAULT_URL = window.parent.RutaNegocio();

//var DEFAULT_URL = 'http://localhost:50988/ViewsAspx/PagePDF/web/compressed.tracemonkey-pldi-09.pdf';
//var DEFAULT_URL = '';
...
var DISABLE_AUTO_FETCH_LOADING_BAR_TIMEOUT = 3000;//5000
...
var SecondaryToolbar = {
    ...
    firstPageClick: function secondaryToolbarFirstPageClick(evt) {
        ...
        if (typeof window.parent.atif_OnPageChange == "function") {
            window.parent.atif_OnPageChange();
        }
        this.close();
    },
...
function webViewerInitialized() {
    ...
    document.getElementById('previous').addEventListener('click',
      function () {
          ...
          //ocultaPaginas(PDFViewerApplication.page);
          if (typeof window.parent.atif_OnPageChange == "function") {        
                  window.parent.atif_OnPageChange();
          }
      });
    ...
    document.getElementById('pageNumber').addEventListener('change', function () {
        ...
        if (typeof window.parent.PaginaIndex == "function") {
            value = window.parent.PaginaIndex();
        }
...
document.addEventListener('pagerendered', function (e) {
    ...
    if (typeof window.parent.PaginaIndex == "function") {
        pageNumber = window.parent.PaginaIndex();
    }
...
//FUNCION QUE RETORNA LA CANTIDAD DE PAGINAS DEL DOCUMENTO
function pageNum() {
    return PDFViewerApplication.pagesCount;
}

function retornaZoom() {
...
function cordenadas(element, x, y) {

    var parent = element.offsetParent;
    var scale = PDFViewerApplication.pdfViewer.currentScaleValue = 0.75;

    var offsetY = ((y * scale) + element.offsetTop) + element.clientTop;
    var offsetX = ((x * scale) + element.offsetLeft) + element.clientLeft;
...
function validaControl(validacion) {
    controlFullScreen = validacion;
}

function salirZoomFullScreen() {
    //var divPadre = parent.document.getElementById("pageContainer" + PDFViewerApplication.page);
    var divPadre = parent.document.getElementById("zoom").getElementsByTagName('div')[0];
    var divHijo = document.getElementById("viewer");
    ...
    parent.$("#zoom").removeClass("modal");
}

function entrarZoomFullScreen() {
    var divPadre = parent.document.getElementById("zoom");
    var divHijo = document.getElementById("pageContainer" + PDFViewerApplication.page);
    //var divClone = divHijo.cloneNode(true);
    //divPadre.appendChild(divClone);

    //$("#navigation a:eq(1)").unbind("click");
    //$("#navigation a:eq(2)").unbind("click");

    divHijo.style.margin = "auto";

    divPadre.appendChild(divHijo);
    divPadre.className = "modal";
    //parent.document.getElementsByTagName("iframe").blur();
         
}

function documento(neg) {
    DEFAULT_URL = neg;
}


function scaleInicial(size) {
    PDFViewerApplication.pdfViewer.currentScaleValue = size;
}

function GotoPages(NumPagina) {
    //ocultaPaginas(NumPagina);

    PDFViewerApplication.page = NumPagina;

    //if (typeof window.parent.atif_OnPageChange == "function") {        
    //        window.parent.atif_OnPageChange();
    //}

    var porcentage = PDFViewerApplication.loadingBar.percent;
    porcentage;
}
...
function ocultaPaginas(NumPagina) {
    //for (var i = 1; i <= PDFViewerApplication.pagesCount; i++) {
    //    if (i != NumPagina) {
    //        document.getElementById("pageContainer" + i).setAttribute('hidden', 'true');
    //    }
    //    else {
    //        document.getElementById("pageContainer" + NumPagina).removeAttribute('hidden');
    //    }
    //}
    
}

function cordenadasCaptura(element, x, y) {
    var parent = element.offsetParent;
    var scale = PDFViewerApplication.pdfViewer.currentScaleValue = 1.25;
    var offsetY = ((y * scale) + element.offsetTop) + element.clientTop;
    var offsetX = ((x * scale) + element.offsetLeft) + element.clientLeft;
    while (parent.clientHeight === parent.scrollHeight) {
        if (parent.dataset._scaleY) {
...
function cordenadasPDFValDacti(element, x, y,escala) {
    var parent = element.offsetParent;
    var scale = PDFViewerApplication.pdfViewer.currentScaleValue = escala;
    var offsetY = ((y * scale) + element.offsetTop) + element.clientTop;
    var offsetX = ((x * scale) + element.offsetLeft) + element.clientLeft;
    while (parent.clientHeight === parent.scrollHeight) {
        if (parent.dataset._scaleY) {
